    ...
    jawBuilder.controller('builder', ['$scope', '$dialog', '$timeout', '$window', function ($scope, $dialog, $timeout, $window) {
        ...
        $scope.add_element_preset = function (item) {
            ...
            d.open().then(function (result) {
                if (result) {
                    ...
                    $scope.custom_element_presets[$scope.custom_element_presets.length - 1].name = result.preset_name;
                    $scope.custom_element_presets[$scope.custom_element_presets.length - 1].content = JSON.stringify($scope.workspace[$scope.workspace.indexOf(item)]);
                    $scope.store_element_presets();
                }
            });
        }
        ...
        $scope.init_element_presets = function () {
            ...
            if ($window.jaw_presets_elements === false || $window.jaw_presets_elements === null || $window.jaw_presets_elements === 'undefined') {
                ...
                $scope.custom_element_presets = $window.jaw_presets_elements;
        ...
        $scope.store_element_presets = function () {
            jQuery.post(
                ajaxurl,
                {
                    'action': 'pb_save_element_presets',
                    'jaw_pb_element_presets': $scope.custom_element_presets
                },
                function (response) {
                    if (debug) {
    ...
    jQuery(document).ready(function () {
        ...
        var switch_pb = function (state, id) {
            ...
            if (state) {
                ...
                jQuery('#wp-content-wrap').removeClass('composer-active');
                jQuery('#jaw_builder').hide();
                ...
                jQuery('.jaw_pb_startup').val('false');
        ...
        if (jQuery('#jaw_page_builder').length > 0) {
            ...
            jQuery('#wp-content-editor-tools')
                ...
                .prepend(
                jQuery('<a id="content-composer" class="wp-switch-editor switch-revo" onclick="switchEditors.switchto(this);"><span></span></a>')
                    .click(function () {
                    switch_pb(true);
                    return false;
                })
