...
// only for metabox layout + sidebars_delect 
        ...
        public static function element_tiles($value, $data = null) {
            ...
            if (isset($value['builder']) && $value['builder'] == 'true') {
                ...
                foreach ((array) $value['options'] as $key => $option) {
                    ...
                    $output .= '<div class="of-radio-tile-img ' . $selected . '" style="background: url(' . $opt . ') repeat scroll 40px 0 rgba(0, 0, 0, 0)" onClick="document.getElementById(\'of-radio-tile-' . $value['id'] . $i . '\').checked = true; " ' . implode(' ', $data_js) . '></div>';
                    $output .= '</span>';
                }
                $output .='</div>';
            }
        ...
        public static function element_builder_bookmark($value, $data = null) {
            ...
            $output .= '<div class="element_button" ng-class="{active: current.bookmark == \'bookmark-' . $value['id'] . '\'}"  ng-click="current.bookmark = \'bookmark-' . $value['id'] . '\'" >' . $icon . $value['name'] . '</div>';
            if (isset($value['content'])) {
                ...
                if (is_array($value['content'])) {
                    foreach ((array) $value['content'] as $sub) {
                        ...
                        $output .= '<li class="' . $sub['id'] . '">' . jwElements::$element($sub) . '</li>';
                    }
                }
...
//CUSTOM
            $output .= "<div  class='pb-settings radio-preset' ng-repeat='(ip,preset) in custom_presets' >";
            $output .= '<label >';
...
//CUSTOM
            $output .= "<div  class='pb-settings radio-preset' ng-repeat='(ip,preset) in custom_element_presets' >";
            $output .= '<label >';
