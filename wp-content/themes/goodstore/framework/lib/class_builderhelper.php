...
if (!class_exists('jwBuilderHelper')) {
    ...
    class jwBuilderHelper {
        ...
        public function __construct() {
            ...
            add_action('wp_ajax_pb_bug_report', array(&$this, 'jaw_ajax_pb_bug_report'));
        ...
        public function jaw_builder_editor() {
            ...
            <div id="jaw-builder-popup"   >
                ...
                if (isset($jaw_builder_options[$_GET['jaw_code']])) {
                    ...
                    <div class="editor_bookmarks"><ul><li ng-repeat="(i, marks) in bookmarks" ng-click="switch_mark(i)">{{marks}}</li></ul></div>
        ...
        /**
         * jaw_ajax_pb_bug_report
         * 
         */
        public function jaw_ajax_pb_bug_report() {
            $bug = get_option('pb_bug');
            if (!isset($bug)) {
                $bug = '';
            }
            $new_bug = '';
            $new_bug .= "\n===================================================\n";
            $new_bug .= "Time: \n";
            $new_bug .= date('Y-m-d H:i:s') . ' UTC';
            $new_bug .= "\n\n";
            if (isset($_POST['comment'])) {
                $new_bug .= "Comment: \n";
                $new_bug .= $_POST['comment'];
                $new_bug .= "\n\n";
            }
            if (isset($_POST['bug'])) {
                $new_bug .= "Log: \n";
            ...
            $new_bug .= "\n===================================================\n===================================================\n===================================================\n\n\n";
            $bug .= $new_bug;
            wp_mail('zivny@ccb.cz', 'Page builder Bug', $new_bug);
            update_option('pb_bug', $bug);
            die();
        }
        ...
        public function jaw_ajax_pb_save_element_presets() {
            $jaw_pb_presets = '';
            if (isset($_POST['jaw_pb_element_presets'])) {
                $jaw_pb_presets = serialize(str_replace('\'', '`', $_POST['jaw_pb_element_presets']));
            }
            if ($jaw_pb_presets != '') {
