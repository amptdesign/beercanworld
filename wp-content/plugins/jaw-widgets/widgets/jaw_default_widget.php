    ...
    function form($instance) {
        ...
        foreach ($this->options as $one_option) {
            ...
            if ($one_option['type'] == 'text') {
                ...
                echo '</select>';
                echo '</p>';
            }else if ($one_option['type'] == 'simplemediapicker') {
                echo '<span ng-controller="widgetAdmin">';
                $a_default = 'init_edit(\'' . $one_option['id'] . '\',json_decode(\'' . addslashes(str_replace('"', '\'', $one_option['values'])) . '\'));';
                echo '<span ng-init="' . $a_default . '" ></span>';
                echo '<div  simplemediapicker ng-model="edit[\'' . $one_option['id'] . '\']" name="' . $one_option['id'] . '" mod="image" ></div>';
                echo '</span>';
            }
        }
    }
