...
//		'total_sales'=>0,
	...
	public function fill_in_data()
	{
		...
		//set custom fields
		if (in_array('wpcf-grade', $this->header)) {
			$key = array_search('wpcf-grade', $this->header);
			$this->meta['wpcf-grade'] = $this->raw_data[$key];
		}
		if (in_array('wpcf-location', $this->header)) {
			$key = array_search('wpcf-location', $this->header);
			$this->meta['wpcf-location'] = $this->raw_data[$key];
		}
		if (in_array('wpcf-can-opening', $this->header)) {
			$key = array_search('wpcf-can-opening', $this->header);
			$this->meta['wpcf-can-opening'] = $this->raw_data[$key];
		}
